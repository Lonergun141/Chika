...
import { PostService } from '@/lib/services/postServices';

export default async function Home() {
	const blogPosts = await PostService.getTopPostsByComments(3);
				...
				<section className="py-16 px-4 sm:px-6 lg:px-8">
					<div className="container mx-auto">
						<div className="text-center mb-12">
							<h2 className="text-6xl font-bold text-foreground mb-4">Most Popular Posts</h2>
							<p className="text-lg text-muted-foreground">
								Posts with the most engagement from our community
							</p>
						</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
							{blogPosts.map((post) => {
								const primaryCategory =
									post.postCategories[0]?.category.name || 'Uncategorized';
